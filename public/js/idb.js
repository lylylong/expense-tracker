...
request.onupgradeneeded = function (event) {
  ...
  // create an object store called "new_input", set it to have an auto incrementing primary key of sorts
...
// This function will be executed if we attempt to submit a new input and there's no internet connection
    ...
    if (getAll.result.length > 0) {
      fetch("/api/transaction/bulk", {
        ...
        .then((serverResponse) => {
          ...
          console.log("All saved data has been submitted!");
        })
        .catch((err) => {
          console.log(err);
        });
...
window.addEventListener("online", uploadInput);
